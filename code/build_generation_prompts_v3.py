...
def main() -> None:
    ...
    parser.add_argument("--ink-topk", type=Path, default=DEFAULT_INK_TOPK)
    parser.add_argument("--china-topk", type=Path, default=DEFAULT_CHINA_TOPK)
    parser.add_argument("--ink-ref", type=Path, default=DEFAULT_INK_REF)
    parser.add_argument("--china-ref", type=Path, default=DEFAULT_CHINA_REF)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--sample-id", type=str, default=None)
    parser.add_argument("--ink-k", type=int, default=5)
    parser.add_argument("--china-k", type=int, default=5)
    parser.add_argument("--primary-label", type=str, default="Ink and wash")
    parser.add_argument("--secondary-label", type=str, default="China_images")
    args = parser.parse_args()

    test_records = load_json(args.test)
    ink_topk_records = load_json(args.ink_topk)
    china_topk_records = load_json(args.china_topk)
    ink_refs = load_json(args.ink_ref)
    china_refs = load_json(args.china_ref)
    ...
    for sample_id in sample_ids:
        ...
        ink_items = (ink_topk_map.get(sample_id, {}).get("top10") or [])[: args.ink_k]
        china_items = (china_topk_map.get(sample_id, {}).get("top10") or [])[: args.china_k]
        prompt = build_prompt(
            sample,
            ink_items,
            china_items,
