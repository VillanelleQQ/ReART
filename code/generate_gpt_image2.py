...
def main() -> None:
    ...
    if args.use_references:
        ...
        if local_reference_paths:
            ...
            if not (args.ink_annotation_json and args.china_annotation_json):
                raise SystemExit(
                    "--ink-annotation-json and --china-annotation-json are required when --use-references is set "
                    "and local references/ is absent"
                )

            annotation_map = build_annotation_map(args.ink_annotation_json, args.china_annotation_json)
